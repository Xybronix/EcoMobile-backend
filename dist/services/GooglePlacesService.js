...
class GooglePlacesService {
    ...
    async searchPlaces(query, country = 'CM') {
        try {
            ...
            const localResults = this.defaultCameroonAreas.filter(area => area.name.toLowerCase().includes(query.toLowerCase()) ||
            ...
            const places = response.data.results
                .filter((place) => Boolean(place.place_id && place.name && place.geometry?.location))
                .map((place) => ({
            ...
            return this.defaultCameroonAreas.filter(area => area.name.toLowerCase().includes(query.toLowerCase()) ||
                area.city.toLowerCase().includes(query.toLowerCase()));
        }
    }
    getDefaultCameroonAreas() {
...
exports.default = GooglePlacesService;
