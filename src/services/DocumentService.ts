...
export class DocumentService {
  ...
  private async saveImage(base64Data: string, userId: string, type: string, side: 'front' | 'back'): Promise<string> {
    ...
    const filename = `${userId}_${type}_${side}_${Date.now()}.${extension}`;
  ...
  async getUserDocumentsStatus(userId: string): Promise<any> {
    const [identityDocuments, residenceProof] = await Promise.all([
    ...
    return {
      identityDocuments,
      residenceProof,
      allDocumentsSubmitted: identityDocuments.length > 0 && residenceProof !== null,
  ...
  async getPendingDocuments(page: number = 1, limit: number = 20): Promise<any> {
    ...
    const [identityDocuments, residenceProofs, total] = await Promise.all([
      prisma.identityDocument.findMany({
      ...
      ]).then(([idCount, rpCount]) => idCount + rpCount)
    ]);
